...
function ViewModifiedData(){
var fwd_url="ajax-view-modified-data";
	$.ajax({
		type : 'POST',
		url : fwd_url,
		dataType : 'json',
			 ...
			 for(var i = 0 ;i<objSize;i++){
				...
				if(pType == "hwp"){
				 ...
				 var html1 = "<h3><a href='#'>" + PType  + "</a></h3>" +
					...
					"<table id='modifyReqTable_"+i+"' class='table table-bordered table-hover normal'> <thead> <tr> <th>SR. NO.</th> <th>Product Name</th> <th> Old Quantity</th> <th>New Quantity</th> <th>Reason</th> <th  style='width: 120px'>Date</th> <th>Requested By</th> <th style='width: 120px'>Approval Date</th> <th>Approval By</th>  </tr> </thead> " +
					"<tbody>";
				 var j = 1;
				  $.each(allDataArray, function (index, element) {
					...
					var approvalBy = element.ManName;
					var approvalDate = element.approvalDate;
					console.log("Env "+uName);
					console.log("Man "+approvalBy);
					html1 +=  "<tr> <td height='10'> <label>"+ j +"</label> </td> <td height='10'> <label>"+ itemName +"</label> </td> <td height='10'> <label>" + cQuantity + "</label> </td> <td height='10'> <label>" + nQuantity + "</label> </td> <td height='10'> <label>"+ reason +"</label> </td> <td height='10'> <label>" + inpDate + "</label> </td> <td height='10'> <label>"+ uName +"</label> </td> <td height='10'> <label>" + approvalDate + "</label> </td> <td height='10'> <label>" + approvalBy + "</label> </td> </tr>";
		 			j++; 
				  });
...
/*function rejectPopup(itemName,cQuantity,nQuantity,regId,modifiedId,inpDate,el) {
	var msg = "Are you sure you want to reject request of modify "+itemName+" quantity from "+cQuantity+" to "+cQuantity+" on date : "+inpDate;
	swal({
...
function approvePopup(itemName,cQuantity,nQuantity,regId,modifiedId,inpDate,el) {
  var msg = "Are you sure you want to approve request of modify "+itemName+" quantity from "+cQuantity+" to "+cQuantity+" on date : "+inpDate;
	swal({
