		...
		var result = JSON.parse(data);
					...
					if(radioValue == "Yes"){					
					 htmlContent1 ="<div class='row'>"				 
		   			...
		   			+"<div class='form-group'>"
						+"<label class='font-weight-bold'>Environment Clearance Number</label>"
						//
						// +"<select class='select2' id='tot_build_area_unit' name='totBuildAreaUnits'>"
							//		+"<option>sq. m.</option>"
							//		+"<option>sq. feet</option>"
							//	 +"</select>"
						+ "<select class='select2' data-placeholder='Select one or more choices'  id='ecId' name='ecId'></div>"
						$.each(result, function (index, element) {
							htmlContent1 +="<option value='"+element.ecId+"'>"+element.ecName+"</option>"									
					});
				    +"</select>"
					+"</div>"
				    +"</div>"
					+"</div>"
				     $("#ecdata").append(htmlContent1);
					makeSelect2();
					 // $("#ecdata").append(htmlContent1);
					//	makeSelect2();
					 }					
					 else{
							$('#ecdata').css('display', 'none');					
							}
		},
